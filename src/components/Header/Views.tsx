...
interface Props {
    hUpdateView(view: CoreStatusOptions): void;
}

const Views: React.FC<Props> = ({ hUpdateView }) => {
    const canReview = useContext(Permission);
    const statuses = ["incomplete"] as CoreStatusOptions[];
    ...
    function getTitle(status: CoreStatusOptions): string {
        let title = "";
    ...
    function hStatusClick(status: CoreStatusOptions) {
        if (canReview) {
